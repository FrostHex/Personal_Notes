...
## 1.4 指针与引用
函数参数传递有方式主要为: 值传递(pass by value)、指针传递(pass by pointer)、引用传递(pass by reference)

#### 1.4.1 指针
```cpp
void function(MyClass* my_object)
{
    my_object->time = 10.0f; // 使用指针访问成员
}

// 调用
MyClass my_object(5.0f);
function(&my_object);  // 传递my_object的地址
```
- `xxx`是指向`Type`类型对象的指针
- 可以为`nullptr`（空指针）
- 需要使用`->`操作符访问成员
- 调用时需要传递地址（使用`&`操作符）
- 可以重新指向其他对象，例如`my_object = &another_object;`
- 指针是一个真实的变量，存储内存地址
#### 1.4.2 引用
```cpp
void function(MyClass& my_object)
{
    my_object.time = 10.0f; // 使用引用直接访问成员
}

// 调用
MyClass my_object(5.0f);
function(my_object);  // 直接传递my_object对象
```
- `xxx`是`Type`类型对象的引用
- 不能为空（必须引用一个有效对象）
- 像使用原始对象一样使用它，用`.`操作符访问成员
- 调用时直接传递对象（不需要`&`操作符）
- 初始化后不能重新绑定到其他对象
- 本质上是原始对象的别名
- (引用在底层通常通过指针实现，但对外部隐藏了这点)
